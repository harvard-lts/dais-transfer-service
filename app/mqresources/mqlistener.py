    ...
    if _reconnect_attempts <= _max_attempts:
        ...
        try:
            if not connection_params.conn.is_connected():
                ...
                logging.debug(
                    f'subscribe_to_listener connecting {connection_params.queue} to with connection id 1 reconnect '
                    f'attempts: {_reconnect_attempts}'
                )
                subscribe_connection(connection_params)
            else:
                logging.debug(
                    f'connect_and_subscibe already connected {connection_params.queue} to with connection id 1 '
                    f'reconnect attempts {_reconnect_attempts}'
                )
        except Exception:
        ...
        logging.error('Maximum reconnect attempts reached for this connection. reconnect attempts: {}'.format(
            _reconnect_attempts))


def subscribe_connection(connection_params):
    if connection_params.ack is not None:
        connection_params.conn.subscribe(destination=connection_params.queue, id=1,
...
class MqListener(stomp.ConnectionListener):
    ...
    def on_message(self, frame):
        ...
        try:
            ...
            raise mqexception.MQException(
                "Incorrect formatting of message detected.  Required JSON but received {} ".format(body))
        ...
        try:
            ...
            # Transfer data
            logging.debug(' TRANSFERRING DATA {} to {}'.format(self.message_data['s3_path'],
                                                               self.message_data['destination_path']))
            transfer_service.transfer_data(self.message_data)
        except Exception:
            transfer_status = mqutils.TransferStatus(
                self.message_data["package_id"],
                "failure",
                self.message_data['destination_path']
            )
...
def initialize_mqlistener():
    ...
    mqlistener = get_mqlistener(connection_params)
...
def get_mqlistener(connection_params):
